...
export default function JobCardsPage() {
  const { jobCards, customers, products, users, currentBranchId, currentUser, addJobCard, updateJobCard } = useStore();
  const role = currentUser?.role;
  const canCreate = canPerformAction(role, 'jobs', 'create');
  const canEditJob = canPerformAction(role, 'jobs', 'edit');
  ...
  const technicians = users.filter(u => u.role === 'technician' && (isAdmin || u.branchId === currentBranchId));
  ...
  }, [jobCards, currentBranchId, statusFilter, search, isAdmin, role, currentUser?.id]);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {(isAdmin || role === 'branch_manager') && techPerformance.length > 0 && (
