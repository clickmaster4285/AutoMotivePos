...
export default function DashboardPage() {
  const { invoices, jobCards, products, customers, refunds, currentBranchId, currentUser, users } = useStore();
  ...
  const branchInvoices = useMemo(() => isAdmin ? invoices : invoices.filter(i => i.branchId === currentBranchId), [invoices, currentBranchId, isAdmin]);
  const branchJobs = useMemo(() => {
    if (isAdmin) return jobCards;
  ...
  }, [jobCards, currentBranchId, isAdmin, isTechnician, currentUser?.id]);
  const branchProducts = useMemo(() => isAdmin ? products : products.filter(p => p.branchId === currentBranchId), [products, currentBranchId, isAdmin]);
  const branchRefunds = useMemo(() => isAdmin ? refunds : refunds.filter(r => r.branchId === currentBranchId), [refunds, currentBranchId, isAdmin]);
  ...
  const getStats = () => {
    ...
    return [
      ...
      { label: 'Customers', value: customers.length, icon: Users, sub: isAdmin ? 'All branches' : 'Branch scope' },
  ...
  const subtitle = isTechnician ? 'Your work overview' : isCashier ? 'Your sales overview' : isAdmin ? 'System-wide overview' : 'Branch overview';
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {(isAdmin || isManager || role === 'inventory_manager' || role === 'service_advisor') && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
          {(isAdmin || isManager || role === 'inventory_manager') && (
            <Card>
      ...
      {(isAdmin || isManager || role === 'inventory_manager') && lowStockCount > 0 && (
