...
export default function ReportsPage() {
  const { invoices, jobCards, products, refunds, branches, currentBranchId, currentUser } = useStore();
  const [reportType, setReportType] = useState('sales');

  const isAdmin = currentUser?.role === 'admin';
  const branchInvoices = isAdmin ? invoices : invoices.filter(i => i.branchId === currentBranchId);
  const branchJobs = isAdmin ? jobCards : jobCards.filter(j => j.branchId === currentBranchId);
  const branchProducts = isAdmin ? products : products.filter(p => p.branchId === currentBranchId);
  const branchRefunds = isAdmin ? refunds : refunds.filter(r => r.branchId === currentBranchId);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <Select value={reportType} onValueChange={setReportType}>
          ...
          <SelectContent>
            ...
            {isAdmin && <SelectItem value="branch_sales">Branch-wise Sales</SelectItem>}
      ...
      {reportType === 'branch_sales' && isAdmin && (
        <Card><CardContent className="p-5">
